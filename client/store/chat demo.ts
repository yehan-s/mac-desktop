...
export const useChatStore = defineStore("chat", () => {
  ...
  const setReceiver = async (receiver_id: number) => {
    ...
    console.log("存储私聊消息的时候触发了一次获取信息");
    // 存储对方的 userInfo
  ...
  // 群视频
  //初始化 PeerConnection
  // async function createMedia(peerName,e) {
  //   const streamTep = ref(null);
  ...
  //     streamTep.value = mediaStream;
  //   } catch (error) {
  //     console.error("获取媒体设备异常", error);
  //     // 请确保$message是一个在setup中正确引用的方法
  //     // that.$message.warning("获取媒体设备异常")
  //   }

  //   return streamTep.value;
  // }
  // 群视频通话
  // 存放自己pc实例
  // const groupPeerList: any[] = [];
  const groupPeerList: Map<string, RTCPeerConnection | null> = new Map();
  // const groupVideoList: any[] = [];
  let groupVideoId = "";
  const groupVideoIsOpen = ref(false);
  const groupVideoHandler = async (userInfo: {
    id: number;
    nickname: string;
    avatar: string;
  }) => {
    groupVideoId = currentChat.groupObject.room + "group";
    // 将user的信息传入myInfo
    myInfo.id = userInfo.id;
    myInfo.nickname = userInfo.nickname;
    myInfo.avatar = userInfo.avatar;
    myInfo.room = groupVideoId;
    // setCallStatus(CallStatus.CALLING);
    groupVideoIsOpen.value = true;
    // 加入房间，会回传视频列表
    // 第一个加入的用户只会有加入房间操作，填入GroupVideoList
    groupJoinVideoRoom({ userInfo: myInfo });
    callStatus.value = CallStatus.CALLING;
    // 每个人都要初始化自己的流与自己的第一个pc
    // peer可以有连接再建
    // groupJoinVideo();
    // 上面一行保留，下面删除
    // if (groupPeerList.length === 0) {
    //   groupPeerList.push(myInfo.nickname);
    // }
    // 1、获取并设置自己的音视频流
    // FIXME: 这里可能只用触发一次，放上面
    await groupInitStream();
  };

  // 后续修改，第一次不用触发
  async function groupJoinVideo() {
    console.log("接听视频通话，方法不一定触发");
    // 参数是roomId + group
    await nextTick();
    // if (!pc) {
    ...
    // await groupInitStream();
    // 2、创建并设置自己的 RTCPeerConnection 实例
    await groupCreatePeerConnection();
    // 添加groupPerrList
    // 如果是第一个则不需要发送offer
    // 后台进行判断
    console.log("尝试获取姓名", [...groupPeerList][groupPeerList.size - 1]);
    await groupSendOffer([...groupPeerList][groupPeerList.size - 1][0]);
  }

  const groupInitStream = async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        video: true,
        audio: false,
      ...
      localStream = stream;
      console.log("群初始化的stream", localStream);
      // 私聊时，被邀请人显示自己的视频流需要单独渲染
      // 什么都没点击时，type时null
      // if (currentChat.sendMessage.type === "private") {
      if (myVideoRef.value) {
        console.log("群己方视频流已加载");
        myVideoRef.value!.srcObject = localStream;
      }
      // }
    } catch {
  ...
  // 加入群视频房间
  const groupJoinVideoRoom = (data: { userInfo: {} }) => {
    // 进入房间的socket
    socket.emit("groupJoinVideoRoom", data);
  };
  // 房间新加入用户
  // 使他们进入同一个房间
  socket.on("groupJoinVideoRoom", (data, groupVideoList) => {
    // 遍历list FIXME: 这里可能有问题
    // data.groupPeerList;
    // 如果自己就是新用户，全部遍历一遍
    console.log("groupJoinVideoRoom", groupVideoList, myInfo.nickname);
    if (groupVideoList[groupVideoList.length - 1] === myInfo.nickname) {
      groupVideoList.forEach((item: string) => {
        if (item === myInfo.nickname) return;
        console.log("看看视频成员", groupVideoList);
        const peerName = [myInfo.nickname, item].sort().join("-");
        // groupPeerList.push(peerName);
        groupPeerList.set(peerName, null);
        // 单纯添加一个房间，用来单独发送信令
        socket.emit("joinRoom", { room: peerName });
        // p2p连接
        groupJoinVideo();
      });
    } else {
      ...
      const peerName = [
        myInfo.nickname,
        groupVideoList[groupVideoList.length - 1],
      ]
        .sort()
        .join("-");
      groupPeerList.set(peerName, null);
  ...
  // socket.on("groupNewUserJoinVideRoom", (data, groupVideoList) => {
  //   groupVideoList.forEach((item: string) => {
  //     if (item === myInfo.nickname) return;
  //     console.log("看看视频成员", groupVideoList);
  //     const peerName = [myInfo.nickname, item].sort().join("-");
  ...
  // 建立点对点连接
  function groupCreatePeerConnection() {
    console.log("createGroupPeerConnection已经初始化----");
    // 这里是同步代码，因此可以这么写
    [...groupPeerList][groupPeerList.size - 1][1]?.signalingState;

    // if (!pc || pc.connectionState === "closed") {
    [...groupPeerList][groupPeerList.size - 1][1]! = new RTCPeerConnection();
    // }

    [...groupPeerList][groupPeerList.size - 1][1]!.onicecandidate =
      onIceCandidate;
    [...groupPeerList][groupPeerList.size - 1][1]!.oniceconnectionstatechange =
      onIceConnectionStateChange;
    [...groupPeerList][groupPeerList.size - 1][1]!.ontrack = onTrack;
    [...groupPeerList][groupPeerList.size - 1][1]!.onicegatheringstatechange =
      onicegatheringstatechange;

    // console.log("给pc传入track", localStream);
    // console.log("看看tracks", localStream.getTracks());
    localStream.getTracks().forEach((track) => {
      [...groupPeerList][groupPeerList.size - 1][1]?.addTrack(
        track,
        localStream
      );
    });
  ...
  // 发送offer，等待n个成员返回
  const groupSendOffer = async (peerName: string) => {
    console.log("发送offer的时候获取一下姓名", peerName);
    try {
      const pc = groupPeerList.get(peerName)!;
      const offer: RTCSessionDescriptionInit = await pc!.createOffer();
      // 接下来，设置己方LOCAL OFFER，并将 offer 发送到对等方
      await pc!.setLocalDescription(offer);
      console.log("发送offer", pc!.signalingState);
      // 改变房间号，这里是异步的，这样可以确定
      myInfo.room = peerName;
      // 发送 offer 到对等方...
      socket.emit("groupOffer", { offer, userInfo: myInfo });
    } catch (error) {
      console.error("创建 offer 失败", error);
    }
  };

  const onGroupReceiveOffer = async (
    offer: RTCSessionDescriptionInit,
    peerName: string
  ) => {
    console.log("收offer的时候打印一下", peerName);
    // if (!pc) return;
    // 设置对方的 REMOTE OFFER
    ...
    try {
      ...
      console.log("接收remote-offer,发送answer", pc.signalingState);
      // console.log("answer这里我看看信息", receiveVideoInfo);
      socket.emit("groupAnswer", { answer, userInfo: myInfo });
    } catch (error) {
      console.warn("接受offer失败", error);
    }
  };
  async function onGroupReceiveAnswer(
  ...
  async function onGroupAddCandidate(candidate: RTCIceCandidateInit) {
    console.log("接收candidate", candidate);
    await pc!.addIceCandidate(candidate);
  }

  socket.on("groupReceive_offer", onGroupReceiveOffer);
  socket.on("groupReceive_answer", onGroupReceiveAnswer);
  socket.on("groupReceive_candidate", onGroupAddCandidate);
